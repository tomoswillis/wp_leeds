		...
		<div class=" xl:flex xl:m-5 xl:justify-between xl:px-5 ">
			<div class="px-5 pt-5 mt-5 md:flex xl:w-3/5 ">
				...
				<div class="xl:w-4/5">
					...
					<p class="font-light">{{$props.current_post.description}}</p>
					<p class="my-5">Average Cost £{{$props.current_post.average_cost}}</p>
					<div class="text-sm flex justify-between md:justify-start lg:flex-col">
						<div class="md:mr-5 lg:flex">
							<div class="flex items-center my-3 lg:mr-5 opacity-25" v-bind:class="{ 'opacity-100': $props.current_post.accessibility.disabled_access_}">
								<span class="material-icons">accessible</span>
								<p class="inline">Disabled Access</p>
							</div>
							<div class="flex items-center my-3 lg:mr-5 opacity-25" v-bind:class="{ 'opacity-100': $props.current_post.accessibility.covid_safe}">
							...
							<div class="flex items-center my-3 lg:mr-5 opacity-25" v-bind:class="{ 'opacity-100': $props.current_post.accessibility.child_friendly}">
						...
						<div class="lg:flex">
							<div class="flex items-center my-3 lg:mr-5 opacity-25" v-bind:class="{ 'opacity-100': $props.current_post.accessibility.private_hire}">
							...
							<div class="flex items-center my-3 lg:mr-5 opacity-25" v-bind:class="{ 'opacity-100': $props.current_post.accessibility.dog_friendly}">
								<span class="material-icons">pets</span>
								<p>Animal Friendly</p>
							</div>
							<div class="flex items-center my-3 lg:mr-5 opacity-25" v-bind:class="{ 'opacity-100': $props.current_post.accessibility.wifi}">
								<span class="material-icons">wifi</span>
								<p>Wifi</p>
							</div>
						</div>
					</div>
					<a :href="$props.current_post.site_url">
			...
			<div class="xl:w-1/3 xl:my-5 xl:mr-5">
				...
				<div class="bg-white px-5 py-5 mt-5 rounded-lg">
					<div class="w-full h-64 bg-black text-white center">
						<mapbox :lat='$props.current_post.location.latitude' :long='$props.current_post.location.longitude'></mapbox>
					</div>
					...
					<a :href="'mailto:' + $props.current_post.contact_information.email">
						<div class="mx-2 my-5">
							<svg fill="none" class="inline" viewBox="0 0 24 24" height="24" width="24" xmlns="http://www.w3.org/2000/svg">
...
const open = DateTime.fromISO(data.open);
let close = DateTime.fromISO(data.close);

if (close < open) {
	close = close.plus({ days: 1 });
}

const opensHours = open.diff(now, 'hours').hours;
const closeHours = close.diff(now, 'hours').hours;

console.log({
	open: {
		display: open.toLocaleString(DateTime.TIME_24_SIMPLE),
		soon: opensHours > 0 && opensHours < 1,
	},
	close: {
		display: close.toLocaleString(DateTime.TIME_24_SIMPLE),
